...
def slidingURL(inpt):
    r = pd.Series([])
    for i in inpt:
        deltaURL = 'http://15.165.6.154:8080/api/images?projectId=' + str(i)
...
```python idxcnt ``` 라는 변수를 설정하여 맨 안쪽 루프문이 끝날때마다 카운트수가 올라게게끔 설정했고
전체  ```python len(rawJson) ``` 만큼 순회하되 ```python idxcnt ``` 부터 ```python idxcnt + 1 ``` 까지, 한개씩 순차적으로 ```python rawJson ```의 인덱스를 크롤링 할것인지 패스할것인지 구현했다.
        ...
        try:
            ...
            if not os.path.isfile(imgDetection):
                ...
                for i in range(idxcnt, idxcnt+1):
                    ...
                    #for j in range(0, 20):
                        deltaimgURL = 'http://15.165.6.154:8080/uploads/' + \
                                    str(inpt[i]) + '/' + \
                                    str(rawJson[i][j]['id'])
...
```python time.sleep(0.05) ```의 시간차를 줘 더 빠르게 이미지를 저장하게끔 했고 문제는 없는듯 했다.
...
다음 글에서 어떻게 위의 문제들을 해결하였는지 적어보겠다,,,